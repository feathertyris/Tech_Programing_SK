...
void ClientGUI::onReadyRead()
{
    ...
    for (const QByteArray &line : lines) {
        ...
        // Фильтруем приветственное сообщение сервера
        if (trimmedLine.contains("Server ready") ||
            trimmedLine.contains("Send JSON with parameters")) {
            qDebug() << "Skipping server welcome message:" << trimmedLine;
            continue;
        }

        // Обрабатываем только валидные JSON‑объекты
        if (trimmedLine.startsWith("{") && trimmedLine.endsWith("}")) {
            qDebug() << "Processing JSON:" << trimmedLine;
            parseResponse(trimmedLine);
        } else {
            qDebug() << "Skipping non-JSON line:" << trimmedLine;
        }
    }
}
